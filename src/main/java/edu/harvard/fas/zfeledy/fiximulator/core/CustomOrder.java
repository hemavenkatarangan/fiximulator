...
public class CustomOrder  extends Order implements Cloneable{
	...
	public CustomOrder clone() {
		CustomOrder order = (CustomOrder) super.clone();
	...
	public CustomOrder(CustomNewOrderSingle message) {
		ID = generateID();
		...
		// SecurityID
				try {
					...
					message.get(custField);
					this.setCustomField(custField.getValue());
				} catch (FieldNotFound ex) {
					ex.printStackTrace();
				}
	}

	private void setCustomField(String value) {
		// TODO Auto-generated method stub
		this.custField =value;
	}

	public CustomOrder(quickfix.fix44.OrderCancelRequest message) {
		// ID = generateID();
		...
		if (oldOrder != null) {
			...
			custField  = ((CustomOrder)oldOrder).getCustomField();
		}
	...
	public CustomOrder(quickfix.fix44.OrderCancelReplaceRequest message) {
		ID = generateID();
